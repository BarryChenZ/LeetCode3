class Solution {
public:
    void remove(string s, vector<string> &result, int start, int lastRemovedIndex, pair<char, char> par) {
        for (int stack = 0, i = start; i < s.size(); i++) {
            ...
            // From HERE, we have ivalid parentheses.
            for (int j = lastRemovedIndex; j <= i; j++) {
                if (s[j] == par.second && (j == lastRemovedIndex || s[j-1] != par.second)) { // ')' parentheses and not consecutive.
                    ...
                    remove(str, result, i, j, par);
                }
            }
            return;
        }
        reverse(s.begin(), s.end());    // reverse string if going right to left.
        if (par.first == '(') remove(s, result, 0, 0, {')', '('});  // give new string
